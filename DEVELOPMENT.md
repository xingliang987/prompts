...
### 2026-07-20 — 新增 C013 编辑前展示思路规则到「AI agent 通用要求.instructions.md」

- **操作目的**：增加一条强制性规则，要求 AI agent 在创建或修改任何文件/代码前，必须先向用户展示编辑思路，说明要做哪些修改、为什么修改及实现方式，征得同意后方可执行。
- **操作方法**：在 `AI agent 通用要求.instructions.md` 的 C012（每次回答后合规检查）之后、项目文件列表之前插入新的 C013 规则项。
- **变更文件**：`AI agent 通用要求.instructions.md`
- **规则内容**：
  - MUST 级：创建或修改任何文件或代码前，先展示编辑思路
  - forbid：未展示思路并征得用户同意就直接执行文件创建/修改
...
### 2026-07-14 — 改造 C000 和 C012 为"每次回答"粒度

- **操作目的**：将 C000（规则复述）和 C012（合规检查）从"会话级"（开始/结束）提升为"回答级"（每次回答前后），强制每轮推理都在思维链中执行，实现"回答前复述→回答中落实→回答后检查"的闭环。
- **操作方法**：在 `AI agent 通用要求.instructions.md` 中分别修改 C000 和 C012 的 `when` 和 `do` 字段。
- **变更要点**：
  - C000：`when: 每次会话开始时` → `when: 每次回答前`，`do` 改为强调每轮推理都显式加载规则上下文
  - C012：`when: 每次会话结束前` → `when: 每次回答结束时`，`do` 改为检查本轮回答是否遵守了每条规则
